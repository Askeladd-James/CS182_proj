...
from model import IndependentTimeModel, UserTimeModel, UMTimeModel, TwoStageMMoEModel, CFModel
...
def train_and_evaluate_model(model_class, model_name, max_userid, max_movieid, train_loader, val_loader, test_data, device, config):
    ...
    logging.info(f"{model_name}参数: 总数={total_params:,}, 可训练={trainable_params:,}")
...
def train_and_evaluate_mmoe_model(model_name, max_userid, max_movieid, train_data, val_data, test_data, device, config):
    ...
    try:
        model, mmoe_history = train_mmoe(
            ...
            num_epochs_per_stage=[8, 12, 8],  # 适配改进模型的轮数
        ...
        for stage_name, stage_history in mmoe_history.items():
            ...
            if 'learning_rates' in stage_history:
                ...
                all_training_history['learning_rates'].extend([0.001] * stage_history['total_epochs'])
            
            if 'epoch_times' in stage_history:
                all_training_history['epoch_times'].extend(stage_history['epoch_times'])
            else:
                all_training_history['epoch_times'].extend([1.0] * stage_history['total_epochs'])
...
def train_and_evaluate_baseline_model(model_name, max_userid, max_movieid, train_data, val_data, test_data, device, config):
    ...
    baseline_train_loader = DataLoader(baseline_train_dataset, batch_size=config['BATCH_SIZE'], shuffle=True, num_workers=0)
    baseline_val_loader = DataLoader(baseline_val_dataset, batch_size=config['BATCH_SIZE'], shuffle=False, num_workers=0)
    ...
    best_model_state, training_history = train_baseline_model_with_metrics(
        model, baseline_train_loader, baseline_val_loader, criterion, optimizer, scheduler, 
        device, config['NUM_EPOCHS'], patience=10
    )
...
def train_baseline_model_with_metrics(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=30, patience=10):
    """专门为Baseline模型的训练函数"""
    best_val_loss = float('inf')
    patience_counter = 0
    best_model_state = None
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(data_path + 'training_comparison_log.txt', encoding='utf-8'),
    ...
    config = {
        ...
        'K_FACTORS': 60,           # 与train.py一致
        'TIME_FACTORS': 16,        # 与train.py一致
        'BATCH_SIZE': 512,         # 与train.py一致
        'NUM_EPOCHS': 30,          # 与train.py一致
        'LEARNING_RATE': 0.001,    # 与train.py一致
        'REG_STRENGTH': 0.001,     # 与train.py一致
        'NUM_EXPERTS': 3           # MMOE专用
    }
    ...
    try:
        baseline_results = train_and_evaluate_baseline_model(
            "Baseline Collaborative Filtering", max_userid, max_movieid,
    ...
    try:
        mmoe_results = train_and_evaluate_mmoe_model(
            "两阶段MMoE模型(带学习率调度)", max_userid, max_movieid,
            train_data, val_data, test_data, config['DEVICE'], config
        )
    ...
    for model_type, results in all_results.items():
        ...
        logging.info(f"  参数数量: {results['model_params']['total_params']:,}{lr_info}")
